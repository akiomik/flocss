...
일반적으로 많이 사용되는 패턴이 있어, 
예를들어 [Bootstrap의 Component 카테고리](http://getbootstrap.com/components/) 등에 보여지는 `button`등이 해당됩니다. 
...
Utility는 Component、Project 레이어의 Object를 무진장 늘어나는 것을 방지하거나, 
또 이것의 Object 자체가 가지고 있지 않아야 할 `margin` 대신에 `.mbs { margin-bottom: 10px; }` 같은 Utility Object를 이용해, 
인접한 모둘과의 간격을 만들겠다는 역활을 담당합니다.
...
[BEM](http://bem.info/) 시스템의 문법에 있는 **Block**、**Element**、**Modifier**로 분류하고 구성된 규칙을 채용합니다.
...
Modifier의 네이밍의 파생 패턴으로, JavaScript로 조작되는 [상태]를 나타내는 Modifier에 대해서는 
SMACSS의 **State** 패턴의 네이밍을 빌리고, `is-*` 프리픽스를 부여해, `.is-active`같은 것도 가능합니다.
...
이 아이디어를 채용하는 경우의 원칙으로써 `.is-active` 란 곳에 룰을 가지게 하는 것은 **금지**합니다. 
이것은 `.is-active` 이것이 가진 룰이 다른 모튤의 Modifier의 스타일을 지져분하게 하는 것을 방지하기 위함입니다.
...
*Note:*  
이것들의 네이밍 규칙은, 당신의 프로젝트가 가진 오리지날의 네이밍 규칙에 따라 카멜케이스 등에 따라가도 상관없습니다만 꼭 **네이밍에 일관성을 유지한다**로 부탁합니다.
...
혹시 Sass나 Stylus 같은 CSS 프리 프로세서나 빌드 툴을 사용한 CSS 파일을 결합해야 할 환경이라면, 
아래처럼 디렉토리를 나눠서 관리하는 것을 권장합니다.
...
모듈 단위로 파일을 분할 함으로써, 페이지 단위 혹은 프로젝트 단위의 모듈의 추가・삭제의 관리가 쉬워집니다.
...
캐스팅은 CSS의 가장 큰 특징 중에 하나입니다. 
운영중에 CSS가 망가지는것은, 섵부른 캐스팅과 셀렉터의 관리에 원인이 있는게 대부분입니다.

FLOCSS에는 마지막에 오는 레이어가 될 정도로 구체적이거나, 캐스팅에 의한 강한 규칙이 필요합니다. 
그 때문에, **레이어의 순서를 앞뒤로 놓지 않습니다.**
...
왜냐면, 그 레이어에 의한 특별히 정한 모듈에 존재하는것 없이, 모듈로써 독립해 재사용 되어야 하고, 혼재시킴으로써 다른 개발자가 예상하지 않은 동작이 되면 안되기 때문입니다. 

예외로써, 
레이어 사이에 걸친 캐스팅, 예를 들어 다음과 같은 **Project 레이어가 Component 레이어 모듈을 변경하는 것은 허용** 합니다.
...
다만 이런 예가 있어도 셀렉터의 캐스팅을 할 필요는 없이 
다음과 같이 Project 레이어의 **Element**이나, Component 레이어의 **Modifier**에 따라 확장할 경우에 따라 해결되는 경우가 있습니다.
...
이와 같이 해결된 경우에는, 세밀도를 강하게 키우는 것을 막을수 있습니다.

한편, 대상의 디자인이 복잡하고 패턴이 너무 많은 케이스일 경우, Element나 Modifier로 코드가 넘쳐나 버리는 경우도 있습니다.
그 경우 모듈 자체가 번잡해 지는 문제가 야기되면 처음에 알려준 샘플 같은 Project 레이어에서의 Component 레이어의 캐스팅을  그 예외로써 허용합니다.
...
이 샘플 외에도, 리스트 계열 모듈에 의한 인접한 요소간의 보더가 필요 한 케이스에는 인접 셀렉터 `E + E {...}`를 이용한 경우도 있고 
":hover"와 같은 유사 클래스나 상태를 표시하는 클래스를 붙히는 경우도 있습니다.
...
CSS 프리 프로세서의 대부분 갖고 셀렉터를 계승하기 위한 Extend는, 원칙 그 모듈에서 완결되어 계승 이외에는 사용을 금지합니다.

레이어나 모듈을 넘어 Extend에 의해 계승한 경우, FLOCSS의 구성・설계는 망가져 캐스팅 룰도 복잡해져 버릴 가능성이 있기 때문입니다.
